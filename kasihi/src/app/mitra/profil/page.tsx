...
export default function MitraProfilPage() {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      // Fetch documents using client (we can just call the action, but client side we can just call action if needed. Actually we have an action for it)
      const { getMitraDocumentsAction } = await import('@/features/mitra_documents/actions');
      const docsRes = await getMitraDocumentsAction(data.id);
    ...
    <div className="flex flex-col flex-1 gap-8 p-4 md:p-8 max-w-5xl mx-auto w-full font-sans antialiased">
      ...
      >
        ...
        <div className="relative z-10 shrink-0 group">
          <div className="w-36 h-36 rounded-[2rem] bg-gray-100 border-4 border-white shadow-xl overflow-hidden relative rotate-3 transition-transform duration-300 group-hover:rotate-0 group-hover:scale-105 cursor-default">
          ...
          <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-brand-evergreen rounded-full border-[3px] border-white shadow-md flex items-center justify-center">
            <div className="w-3 h-3 bg-white rounded-full animate-ping opacity-75 absolute" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        >
          <div className="flex items-center justify-between border-b border-gray-100 pb-4">
            <h2 className="font-bold text-[#1A2332] text-xl flex items-center gap-2">
              <ShieldCheck className="text-brand-evergreen" /> Dokumen Verifikasi Mitra
            </h2>
          </div>
