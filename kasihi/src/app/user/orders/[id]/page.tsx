...
export default async function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isPaid = currentIdx >= 1 || (order.status === 'pending_payment' && hasProof);
  const isVerified = currentIdx >= 1; // waiting_mitra = sudah diverifikasi admin
  const isRunning = currentIdx >= 2 || order.status === 'service_done';
