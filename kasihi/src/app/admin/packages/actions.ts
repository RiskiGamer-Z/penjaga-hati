...
export async function addPackageAction(data: { name: string, description: string, duration_hours: number, price_per_hour: number, base_price: number }) {
  try {
    const adminClient = createAdminClient()
    const { error } = await adminClient.from('service_packages').insert(data)
...
export async function updatePackageAction(id: string, data: { name: string, description: string, duration_hours: number, price_per_hour: number, base_price: number }) {
  try {
    const adminClient = createAdminClient()
    const { error } = await adminClient.from('service_packages').update(data).eq('id', id)
