...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  // Config rute yang diperbolehkan untuk masing-masing admin
  const admin1Allowed = [
    "/admin/dashboard",
    "/admin/mitra",
    "/admin/pengguna",
    "/admin/reviews",
    "/admin/pesanan" // Allow access to order details for CS purposes
  ];

  const admin1MenusHidden = [
    "/admin/pesanan",
    "/admin/verifikasi"
  ];
  ...
  const isRouteAllowed = (href: string, email: string) => {
    if (!email) return true; // Tunggu hingga email admin berhasil dimuat
    if (email === 'admin@penjagahati.com') {
      return admin1Allowed.some(p => href === p || href.startsWith(p + '/'));
    }
    if (email === 'keuangan@penjagahati.com') {
      return admin2Allowed.some(p => href === p || href.startsWith(p + '/'));
    }
    return true; // default
  };

  const isMenuAllowed = (href: string, email: string) => {
    if (!email) return true;
    if (email === 'admin@penjagahati.com') {
      // Hide specific menus for admin utama
      if (admin1MenusHidden.includes(href)) return false;
      return admin1Allowed.some(p => href === p || href.startsWith(p + '/'));
    }
    if (email === 'keuangan@penjagahati.com') {
      return admin2Allowed.some(p => href === p || href.startsWith(p + '/'));
  ...
  // Filter bagian navigasi sidebar berdasarkan perizinan
  const filteredSections = sections.map(section => ({
    ...section,
    items: section.items.filter(item => isMenuAllowed(item.href, email))
  })).filter(section => section.items.length > 0);

  // Periksa apakah rute saat ini diperbolehkan
  const isCurrentPathAllowed = email ? isRouteAllowed(pathname, email) : true;
        ...
        >
          <div className="flex flex-col h-full py-6">
            ...
            <div className={`flex items-center px-6 gap-3 mb-8 transition-all duration-300 ${!isSidebarExpanded && "lg:px-5 lg:justify-center"}`}>
              <div className="relative w-10 h-10 bg-white rounded p-1.5 shrink-0 border border-white/10">
              ...
              <div className={`flex flex-col overflow-hidden transition-all duration-300 ${showText ? "opacity-100 w-auto" : "opacity-0 w-0 lg:hidden"}`}>
                <span className="font-sans font-bold text-white text-lg leading-5 tracking-tight">Kasihi</span>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#F9FAFB]">
            {!isCurrentPathAllowed ? (
              <div className="flex flex-col items-center justify-center min-h-[70vh] p-6 text-center">
                <div className="w-16 h-16 bg-red-50 text-red-500 rounded-2xl flex items-center justify-center mb-4 border border-red-100 shadow-sm animate-bounce">
                  <ShieldAlert size={32} />
                </div>
                <h2 className="text-xl font-black text-brand-navy">Akses Dibatasi</h2>
                <p className="text-slate-500 text-sm max-w-md mt-1">
                  Maaf, akun Anda ({email}) tidak memiliki izin akses untuk halaman <strong>{pathname}</strong>. Halaman ini dikhususkan untuk peran admin lainnya.
                </p>
                <button 
                  onClick={() => router.push('/admin/dashboard')}
                  className="mt-6 px-5 py-2.5 bg-brand-navy text-white font-medium text-sm rounded shadow-sm hover:bg-blue-900 transition-colors"
                >
