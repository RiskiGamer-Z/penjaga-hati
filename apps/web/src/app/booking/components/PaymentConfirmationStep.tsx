import { ArrowLeft, CheckCircle2, CreditCard, Loader2, UploadCloud, FileText } from "lucide-react";
...
export default function PaymentConfirmationStep({
  ...
  // Preview bukti transfer (khusus gambar). PDF ditampilkan sebagai ikon.
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const isImage = formData.paymentProof?.type.startsWith("image/");

  useEffect(() => {
    if (formData.paymentProof && isImage) {
      const url = URL.createObjectURL(formData.paymentProof);
      setPreviewUrl(url);
      return () => URL.revokeObjectURL(url);
    }
    setPreviewUrl(null);
  }, [formData.paymentProof, isImage]);
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="col-span-2">
              <p className="text-slate-400 text-xs mb-1.5">Mitra Ditunjuk</p>
              <div className="flex items-center gap-3">
                {selectedMitra?.users?.avatar_url ? (
                  <img
                    src={selectedMitra.users.avatar_url}
                    alt={selectedMitra.users?.full_name || "Mitra"}
                    className="w-10 h-10 rounded-full object-cover border border-slate-200 shrink-0"
                  />
                ) : (
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white shrink-0 ${
                    selectedMitra?.gender === "Perempuan" ? "bg-pink-500" : "bg-blue-500"
                  }`}>
                    {(selectedMitra?.users?.full_name || "M").charAt(0)}
                  </div>
                )}
                <div className="min-w-0">
                  <p className="font-bold text-indigo-950 truncate">{selectedMitra?.users?.full_name || "-"}</p>
                  {selectedMitra?.specializations && selectedMitra.specializations.length > 0 && (
                    <p className="text-[11px] text-slate-500 truncate">
                      {selectedMitra.specializations.slice(0, 2).join(", ")}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div>
              ...
              <p className="font-black text-rose-500 text-base">
                Rp {Number(selectedPackage?.base_price ?? selectedPackage?.price_per_unit ?? 0).toLocaleString('id-ID')}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-indigo-100 mb-8 shadow-sm">
        ...
        <div className="bg-indigo-50/60 rounded-xl p-4 mb-5 border border-indigo-100">
          <p className="text-xs text-slate-600 mb-2">Silakan lakukan transfer sesuai nominal tagihan ke rekening resmi Penjaga Hati:</p>
          <div className="font-mono font-black text-xl text-indigo-950">BCA — 8830 1928 44</div>
          <p className="text-xs text-indigo-600 font-bold mt-1">a.n. PT Penjaga Hati Indonesia</p>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <label className={`flex flex-col items-center justify-center w-full min-h-[8rem] border-2 border-dashed rounded-2xl cursor-pointer transition-all overflow-hidden ${
            formData.paymentProof ? 'border-emerald-500 bg-emerald-50/50' : 'border-slate-300 bg-slate-50 hover:bg-slate-100'
          }`}>
            <div className="flex flex-col items-center justify-center py-5 px-4 w-full">
                ...
                <>
                  {isImage && previewUrl ? (
                    <img
                      ...
                      className="w-24 h-24 object-cover rounded-xl border border-emerald-200 shadow-sm mb-3"
                    />
                  ) : (
                    <div className="w-16 h-16 rounded-xl bg-emerald-100 flex items-center justify-center mb-3">
                  ...
                  <p className="text-[10px] text-emerald-600/70 mt-1">Klik untuk mengganti file</p>
                </>
              ) : (
                <>
                  <UploadCloud className="w-8 h-8 mb-2 text-slate-400" />
                  <p className="mb-1 text-xs text-slate-600"><span className="font-bold text-emerald-600">Klik untuk unggah file</span> atau drag & drop</p>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-slate-100">
        ...
        {formData.paymentProof ? (
          <button
            ...
            className="flex items-center rounded-full py-3.5 px-8 gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-lg shadow-emerald-200 disabled:opacity-50 transition-all"
          >
            {isSubmitting ? (
              <>
