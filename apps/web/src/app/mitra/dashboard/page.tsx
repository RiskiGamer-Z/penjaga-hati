...
export default async function MitraDashboardPage() {
  ...
  if (!mitra) {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-20 text-center">
        ...
        <p className="text-gray-500 mt-2">Pastikan Anda telah terdaftar sebagai mitra Penjaga Hati.</p>
  ...
  const activeStatuses = ['waiting_mitra', 'accepted', 'in_transit', 'arrived', 'in_progress'];
  const finishedStatuses = ['completed', 'service_done'];
  const activeOrders = safeOrders.filter(o => activeStatuses.includes(o.status));
  const finishedOrders = safeOrders.filter(o => finishedStatuses.includes(o.status));
  ...
  return (
    <div className="flex flex-col flex-1 gap-6 p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Left Column: Daftar Tugas (Col Span 2) */}
        <div className="lg:col-span-2 flex flex-col gap-6">
          {/* Tugas Aktif */}
          <div className="bg-white rounded-2xl border border-[#E8F0F0] shadow-sm flex flex-col overflow-hidden">
            <div className="flex items-center justify-between py-5 px-6 border-b border-[#E8F0F0] bg-gray-50/50">
              <div className="flex items-center gap-3">
                <h2 className="font-bold text-[#1A2332] text-lg">Tugas Aktif</h2>
                {activeOrders.length > 0 && (
                  <div className="rounded-full py-1 px-3 bg-blue-100 border border-blue-200">
                    <span className="font-bold text-blue-600 text-[11px]">{activeOrders.length} Berjalan</span>
                  </div>
                )}
              </div>
            ...
            <div className="divide-y divide-[#E8F0F0]">
              {activeOrders.length > 0 ? activeOrders.slice(0, 5).map((order) => {
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`rounded-md py-1 px-2.5 text-[10px] font-bold uppercase tracking-wider border ${
                          ...
                          order.status === 'waiting_mitra' ? 'bg-orange-50 text-orange-600 border-orange-100' :
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center px-4">
                  <Clock size={44} className="text-gray-200 mb-4" />
                  <p className="text-gray-500 font-medium">Belum ada tugas aktif.</p>
                  <p className="text-gray-400 text-sm mt-1">Tugas baru akan muncul di sini setelah ditetapkan.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E8F0F0] shadow-sm flex flex-col overflow-hidden">
            <div className="flex items-center justify-between py-5 px-6 border-b border-[#E8F0F0] bg-gray-50/50">
              <div className="flex items-center gap-3">
                <h2 className="font-bold text-[#1A2332] text-lg">Tugas Selesai</h2>
                {finishedOrders.length > 0 && (
                  <div className="rounded-full py-1 px-3 bg-emerald-100 border border-emerald-200">
                    <span className="font-bold text-brand-evergreen text-[11px]">{finishedOrders.length} Tuntas</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="divide-y divide-[#E8F0F0]">
              {finishedOrders.length > 0 ? finishedOrders.slice(0, 5).map((order) => {
                const hospital: any = Array.isArray(order.hospitals) ? order.hospitals[0] : order.hospitals;
                const pkg: any = Array.isArray(order.service_packages) ? order.service_packages[0] : order.service_packages;
                return (
                  <div key={order.id} className="flex flex-col md:flex-row md:items-center justify-between p-6 gap-6 hover:bg-gray-50/50 transition-colors">
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-gray-500 text-sm">#{order.id.slice(0, 8).toUpperCase()}</span>
                        <span className={`rounded-md py-1 px-2.5 text-[10px] font-bold uppercase tracking-wider border ${
                          order.status === 'completed' ? 'bg-emerald-50 text-brand-evergreen border-emerald-100' : 'bg-purple-50 text-purple-600 border-purple-100'
                        }`}>
                          {order.status === 'completed' ? 'Selesai' : 'Menunggu Konfirmasi'}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2 text-[#6B7B8D] text-[13px] mt-1">
                         <MapPin size={14} className="text-gray-400" />
                         <span>{hospital?.name || 'Lokasi tidak tersedia'} • {pkg?.name}</span>
                      </div>
                    </div>

                    <div className="flex items-center gap-3 shrink-0">
                      <Link href={`/mitra/pesanan/${order.id}`} className="flex-1 md:flex-none flex items-center justify-center gap-2 rounded-xl py-2.5 px-6 border border-gray-200 text-gray-600 font-bold text-sm hover:bg-gray-50 transition-colors">
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center px-4">
                  ...
                  <p className="text-gray-500 font-medium">Belum ada tugas yang selesai.</p>
                  <p className="text-gray-400 text-sm mt-1">Riwayat tugas tuntas akan tampil di sini.</p>
                </div>
              )}
            </div>
          </div>
