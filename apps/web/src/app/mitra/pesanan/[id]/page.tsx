...
import WorkflowPhotosHistory from "../components/WorkflowPhotosHistory";
import { 
    ...
    <div className="flex flex-col flex-1 gap-6 p-4 md:p-8 max-w-6xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            {/* Absensi Berkala (hanya saat pendampingan berlangsung) */}
            {order.status === 'in_progress' && mitra && (
              <AttendanceCard orderId={order.id} mitraId={mitra.id} />
            )}

            {/* Foto & Catatan Progres */}
            {['arrived', 'in_progress', 'service_done', 'completed'].includes(order.status) && (
              <WorkflowPhotosHistory orderId={order.id} refreshKey={order.photo_count || 0} />
            )}
          </div>
                  ...
                  <div className="p-6 flex flex-col gap-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col gap-0.5">
                         ...
                         <span className="text-gray-500 text-xs font-medium">Klien Penjaga Hati</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {clientProfile.phone ? (
                        <a 
                          href={`https://wa.me/${clientProfile.phone.replace(/[^0-9]/g, '').startsWith('0') ? '62' + clientProfile.phone.replace(/[^0-9]/g, '').slice(1) : clientProfile.phone.replace(/[^0-9]/g, '')}?text=Halo%20${encodeURIComponent(clientProfile.full_name)},%20saya%20mitra%20pendamping%20dari%20Penjaga%20Hati%20untuk%20pesanan%20%23${order.order_number}`} 
                          target="_blank"
