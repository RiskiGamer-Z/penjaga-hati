...
import { User, Mail, Lock, Phone, ShieldCheck, Zap, ArrowRight, Star, Heart, Loader2 } from "lucide-react";
    ...
    <div className="min-h-screen flex font-sans antialiased">
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-1/4 right-1/4 w-64 h-64 bg-emerald-700/30 rounded-full blur-3xl opacity-50" />
          <div className="absolute top-2/3 left-1/4 w-48 h-48 bg-teal-600/20 rounded-full blur-3xl opacity-40" />
          <div className="absolute bottom-1/5 right-1/3 w-56 h-56 bg-emerald-500/20 rounded-full blur-3xl opacity-30" />
        </div>
        ...
        <div className="flex flex-col gap-8 relative z-10">
          <div className="flex flex-col gap-5">
            <h1 className="tracking-tight font-bold text-white text-4xl leading-[1.2]">
              Bergabung & Lindungi{" "}
              <span className="text-emerald-400">Orang Tersayang</span> Anda
            </h1>
            <p className="text-white/70 text-[16px] leading-relaxed max-w-sm">
              Buat akun untuk mulai memesan jasa pendamping pasien profesional di rumah sakit — cepat, aman, terpercaya.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3">
            {[
              { icon: ShieldCheck, label: "Privasi & Keamanan Data", desc: "Data Anda terenkripsi dan terlindungi", color: "text-emerald-400", bg: "bg-emerald-500/15", border: "border-emerald-500/30" },
              { icon: Zap, label: "Daftar Hanya 1 Menit", desc: "Proses sederhana tanpa biaya pendaftaran", color: "text-amber-400", bg: "bg-amber-500/15", border: "border-amber-500/30" },
              { icon: Star, label: "Layanan Premium", desc: "Mitra berpengalaman, rating rata-rata 4.9 bintang", color: "text-teal-400", bg: "bg-teal-500/15", border: "border-teal-500/30" },
              { icon: Heart, label: "Pendampingan Penuh Kasih", desc: "Kami hadir saat Anda membutuhkan kami paling", color: "text-rose-400", bg: "bg-rose-500/15", border: "border-rose-500/30" },
            ].map((item, i) => (
              <div
                key={i}
        ...
        <Link href="/" className="flex items-center gap-3 relative z-10 hover:opacity-90 transition-opacity group mt-auto pt-8 border-t border-white/10">
          <div className="relative w-10 h-10 bg-white rounded-xl p-1.5 shadow-lg">
            <Image src="/logo.png" alt="Penjaga Hati Logo" fill className="object-contain" />
          </div>
          <div className="flex flex-col">
            <span className="font-bold text-white text-base">Penjaga Hati</span>
      ...
      <div className="flex flex-col items-center justify-center flex-1 py-12 px-6 sm:px-12 lg:px-16 relative overflow-y-auto">
        ...
        <Link href="/" className="absolute top-8 left-6 lg:hidden flex items-center gap-2 hover:opacity-80 transition-opacity z-10">
          ...
          <span className="font-bold text-lg text-white">Penjaga Hati</span>
        </Link>
        ...
        <div className="flex flex-col w-full max-w-sm gap-7 relative z-10 mt-20 lg:mt-0">
          {/* Header */}
          <div className="flex flex-col gap-3">
            ...
            <h2 className="tracking-tight font-bold text-white lg:text-brand-navy text-[26px]">
              Daftar Sekarang 🎉
            </h2>
            <p className="text-white/60 lg:text-[#6B7B8D] text-[14px]">
              Bergabung dan dapatkan layanan terbaik kami
            </p>
          </div>
          ...
          <form action={signup} className="flex flex-col gap-4">
            {/* Field config — warna icon sesuai tema dashboard */}
            {[
              {
                label: "Nama Lengkap", name: "fullName", type: "text",
                autocomplete: "name", placeholder: "Masukkan nama lengkap",
                Icon: User, iconColor: "text-emerald-400", iconBg: "bg-emerald-500/20"
              },
              {
                label: "Email", name: "email", type: "email",
                autocomplete: "email", placeholder: "nama@email.com",
                Icon: Mail, iconColor: "text-teal-400", iconBg: "bg-teal-500/20"
              },
              {
                label: "Nomor Telepon", name: "phone", type: "tel",
                autocomplete: "tel", placeholder: "08xxxxxxxxxx",
                Icon: Phone, iconColor: "text-amber-400", iconBg: "bg-amber-500/20"
              },
              {
                label: "Password", name: "password", type: "password",
                autocomplete: "new-password", placeholder: "Minimal 8 karakter",
                Icon: Lock, iconColor: "text-emerald-400", iconBg: "bg-emerald-500/20"
              },
            ].map((field) => (
              <div key={field.name} className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center rounded-2xl py-3.5 px-4 gap-3 bg-white/10 lg:bg-gray-50 border-2 border-white/20 lg:border-gray-200 focus-within:border-emerald-500 focus-within:ring-4 focus-within:ring-emerald-500/10 transition-all backdrop-blur-sm">
                  <div className={`w-8 h-8 rounded-xl ${field.iconBg} flex items-center justify-center shrink-0`}>
                    <field.Icon className={field.iconColor} size={16} strokeWidth={2.5} />
                  </div>
                  <input
                    type={field.type}
                    name={field.name}
            ...
            <div className="mt-2">
              <SubmitButton label="Daftar Sekarang" loadingLabel="Mendaftarkan akun..." />
            </div>
          </form>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] text-white/40 lg:text-gray-400 font-bold uppercase tracking-wider">atau daftar dengan</span>
            <div className="h-px bg-white/10 lg:bg-gray-200 flex-1" />
          </div>
          ...
          <div className="flex items-center justify-center gap-1.5">
            <span className="text-white/50 lg:text-[#6B7B8D] text-sm">Sudah punya akun?</span>
            <Link href="/auth/login" className="font-bold text-emerald-400 hover:text-emerald-300 text-sm transition-colors flex items-center gap-1">
