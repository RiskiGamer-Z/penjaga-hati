    ...
    <nav className={`fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-gray-100 transition-shadow duration-300 ${scrolled ? 'shadow-lg' : 'shadow-none'}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          {/* Logo - Wordmark Penjaga Hati (hitam & merah) */}
          <Link href="/" className="flex-shrink-0 flex items-center gap-4 group">
            <Image
              src="/logo.png"
              alt="Penjaga Hati Logo"
              width={56}
              height={56}
              className="shrink-0"
              priority
            />
            <span className="font-extrabold text-2xl tracking-tight leading-none select-none transition-transform duration-300 group-hover:scale-[1.02]">
              <span className="text-black">Penjaga</span>{" "}
              <span className="text-red-600">Hati</span>
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <Link href="/#layanan" className="relative text-sm font-medium text-gray-600 hover:text-brand-evergreen transition-colors group">
              Layanan
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-brand-evergreen transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></span>
            </Link>
            <Link href="/#paket" className="relative text-sm font-medium text-gray-600 hover:text-brand-evergreen transition-colors group">
              Paket & Harga
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-brand-evergreen transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></span>
            </Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-b border-gray-100 shadow-lg absolute w-full animate-in slide-in-from-top-4 duration-200">
          <div className="px-4 pt-2 pb-6 space-y-1">
            ...
            <Link href="/#layanan" onClick={() => setIsMobileMenuOpen(false)} className="block px-3 py-3 text-base font-medium text-brand-navy hover:text-brand-evergreen hover:bg-gray-50 rounded-lg">
              Layanan
            </Link>
            <Link href="/#paket" onClick={() => setIsMobileMenuOpen(false)} className="block px-3 py-3 text-base font-medium text-brand-navy hover:text-brand-evergreen hover:bg-gray-50 rounded-lg">
              Paket & Harga
            </Link>
